...
if(isset($_GET['menu']) AND isset($_GET['submenu'])) {
  ...
  if($theSubMenu['sort_by'] != "") { $order_by = explode(" ", $theSubMenu['sort_by']); }
  ...
  if($theMenu['sort_by'] != "") { $order_by = explode(" ", $theMenu['sort_by']); }
...
if(isset($_GET['parent_id'])) {
  if($_GET['parent_id']['field'] != "" AND $_GET['parent_id']['id'] != "") {
    ...
    if($theOptionSetting) {
      $theValue = global_select_field($theOptionSetting['get_from_table'], $theOptionSetting['show_field'], "id = '".$_GET['parent_id']['id']."'");
      $parentMenu = global_select_single("m3nu_4dm1n", "*", "top_or_left = 'left' AND table_name_no_prefix = '".$theOptionSetting['get_from_table']."'");
      $topMenu    = global_select_single("m3nu_4dm1n", "*", "top_or_left = 'top' AND id = '".$parentMenu['top_parent_id']."'");
      $title_h2 = "<a style='text-decoration:underline;' href='view.php?menu=".$topMenu['url']."&submenu=".$parentMenu['url']."'>".$parentMenu['name'].'</a> ['.$theValue.'] &raquo '.replace_to_space($theMenu['name']);
    }
  }
}
