...
(function($) {
    ...
    var DH = {
        ...
        userInit: function() {
            ...
            $(document).on('click', '[data-rel=loginModal_checkout]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('#userregisterModal').length) {
                    $('#userregisterModal').modal('hide');
                ...
                if ($('#userloginModal_checkout').length) {
                    $('#userloginModal_checkout').modal('show');
                }
            });
            ...
            <!-- for ADDRESS BOOK -->
            $(document).on('click', '[data-rel=addAddressModal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.addAddress-modal').length) {
                    $('.modal.addAddress-modal').modal('show');
                }
            });
            $(document).on('click', '[data-rel=editAddressModal]', function(e) {
                var id_addressbook = this.id;
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.editAddress-modal-' + id_addressbook).length) {
                    $('.modal.editAddress-modal-' + id_addressbook).modal('show');
                }
            });
            $(document).on('click', '[data-rel=editBillingModal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.editbilling-modal').length) {
                    $('.modal.editbilling-modal').modal('show');
                }
            });
            <!-- for ORDER HISTORY -->
            $(document).on('click', '[data-rel=orderDetailModal]', function(e) {
                var id_record = this.id;
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.statusDetail-modal-' + id_record).length) {
                    $('.modal.statusDetail-modal-' + id_record).modal('hide');
                }
                if ($('.modal.orderDetail-modal-' + id_record).length) {
                    $('.modal.orderDetail-modal-' + id_record).modal('show');
                }
            });
            $(document).on('click', '[data-rel=statusDetailModal]', function(e) {
                var id_record = this.id;
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.orderDetail-modal-' + id_record).length) {
                    $('.modal.orderDetail-modal-' + id_record).modal('hide');
                }
                if ($('.modal.statusDetail-modal-' + id_record).length) {
                    $('.modal.statusDetail-modal-' + id_record).modal('show');
                }
            });
            <!-- for MEASUREMENT GUIDE -->
            $(document).on('click', '[data-rel=measurementModal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.measurement-modal').length) {
                    $('.modal.measurement-modal').modal('show');
                }
            });
            $(document).on('click', '[data-rel=custombordirmodal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.custombordir-modal').length) {
                    $('.modal.custombordir-modal').modal('show');
                }
            });
            <!-- TRACK ORDER & CONFIRM PAYMENT -->
            $(document).on('click', '[data-rel=trackOrderModal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.trackOrder-modal').length) {
                    $('.modal.trackOrder-modal').modal('show');
                }
            });
            $(document).on('click', '[data-rel=confirmPaymentModal]', function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($('.modal.confirmPayment-modal').length) {
                    $('.modal.confirmPayment-modal').modal('show');
                }
            });
        },
