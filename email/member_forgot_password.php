...
/*Mod Variable Member*/
$id_member 				= $data['id'];
$data_member			= global_select_single('member','*',"`id` = '$id_member'");
$username 				= $data_member['fullname'];
$email 					= $data_member['email'];
$token_reset			= $data_member['token_reset'];
$subject 				= "Member Forgot Passowrd";

/*Mod Variable Email*/
$data_email 	= global_select_single("email_content","*","`for` = 'Forgot_password'");
$subject 		= $data_email['subject_id'];
$title 			= $data_email['title_id'];
$content 		= $data_email['content_id'];
ob_start();
?>
							 ...
							 <table cellpadding="0" cellspacing="0" border="0" id="white-wrap" bgcolor="#ffffff" style="margin-bottom:10px;" width="100%">
								<tr>
									<td style="padding:15px;">   
										...
										<p>&nbsp;</p>
										<p><?php echo str_replace("[LINK]", '<strong><a href="'.$website_config.'/'.$curr_lang.'/restore-password/'.$token_reset.'">Click on this Link</a></strong>', $content) ?></p>
										<p>Cheers, </p>
